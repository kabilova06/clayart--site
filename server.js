...
// Создаём таблицы, если их нет (НЕ УДАЛЯЕМ)
db.exec(`
...
try { db.exec(`ALTER TABLE products ADD COLUMN is_bestseller INTEGER DEFAULT 0`); } catch(e) {}
try { db.exec(`ALTER TABLE products ADD COLUMN is_new INTEGER DEFAULT 0`); } catch(e) {}

// Добавляем начальные товары ТОЛЬКО если таблица пустая
const count = db.prepare('SELECT COUNT(*) as count FROM products').get();
if (count.count === 0) {
    console.log('📦 База пустая. Добавляем 25 товаров...');
    ...
    console.log('✅ Добавлены начальные 25 товаров');
} else {
    console.log(`✅ В базе уже есть ${count.count} товаров. Ничего не удаляем и не перезаписываем.`);
}
